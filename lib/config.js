...
nconf.init = function (configFile) {
    ...
    // 2) command line arguthorization or env var
...
nconf.openidParameters = function () {
    if (!openidParams) {
        var proxyOptions = nconf.proxyOptions();
        var domain = app.config.get('domain');
        var baseUrl = proxyOptions['openid.' + domain] + '/';
        openidParams = {
            ...
            callbackURL: proxyOptions[domain] + '/',
...
nconf.proxyOptions = function () {
    if (!proxyOpts) {
        ...
        var domain = app.config.get('domain');
        var tls = app.config.get('enable_tls');
