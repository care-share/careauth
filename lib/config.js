...
nconf.init = function (configFile) {
    ...
    // this filter defines what proprties should be returned in user queries (omits password hash)
    var userFilter = '_id email name_first name_last roles origin fhir_id phone picture contact_pref';
    app.config.set('user_filter', userFilter);
};
...
nconf.isDisabled = function (key) {
    return !app.config.isEnabled(key);
};
...
nconf.configTls = function () {
    if (app.config.isEnabled('use_tls')) {
        ...
        if (app.config.isEnabled('use_suite_b')) {
            // The NSA Suite B standard requires use of the Transport Layer Security (TLS) v1.1 or v1.2 protocols
            options.secureProtocol = "TLSv1_2_method";
            // Suite B allows for two encryption levels, "128-bit" or "192-bit"
            // Here we enforce ciphers at the maximum "192-bit" level
            options.ciphers =
                "ECDHE-ECDSA-AES256-GCM-SHA384:" + // TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384, should use elliptic curve certificates
                "ECDHE-ECDSA-AES256-SHA384:" +     // TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA384, should use elliptic curve certificates
                "ECDHE-ECDSA-AES256-SHA";          // TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA, should use elliptic curve certificates
        }
    ...
    if (!jwtParams) {
        jwtParams = {
            ...
            options: {
                ...
                expiresIn: app.config.get('jwt_ttl') / 1000
            }
        };
    }
    return jwtParams;
...
// returns the proxy table for the reverse proxy listener
var proxyOpts;
nconf.proxyOptions = function () {
    if (!proxyOpts) {
        ...
        if (!proxyFHIR)
            proxyFHIR = proto + '://' + domain + ':8080/baseDstu2';
        if (!proxyOpenID)
            proxyOpenID = proto + '://' + domain + ':8888';
        if (!proxyAPI)
            proxyAPI = proto + '://' + domain + ':3001';
