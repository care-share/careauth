...
function Account(mongoose, config) {
    var jwt_params = config.jwtParameters();
    ...
    Account.statics.encode = function(data) {
        return jwt.sign(data, jwt_params.secret, jwt_params.options);
    };
    Account.statics.decode = function(data) {
        return jwt.decode(data, jwt_params.secret, jwt_params.options);
    };
