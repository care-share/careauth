...
function FhirCloner(baseUrlString, maxRecursion, maxObjectsPerResponse, inputMethod)
{
    this.maxRecursion = (maxRecursion == null) ? 10 : maxRecursion;
    this.maxObjectsPerResponse = (maxObjectsPerResponse == null) ? 10 : maxObjectsPerResponse;
...
FhirCloner.prototype.clonePatientMain = function(inputPatientId, outputPatientId) {
    ...
    console.log("%%% output (new) patient id: " + this.outputPatientId);
    ...
    console.log("switch inputMethod: " + this.inputMethod);
    ...
    switch (this.inputMethod)
    {
        case "file":
            ...
            var outputMap = this.processLoadedData(inputMap, oldToNewIdMap);
        ...
        case "server":
            ...
            pullDataResultPromise = this.pullDataFromFhir(patientUrlString, inputMap, 0, oldToNewIdMap);
...
var targetPatients = patientList.filter(function(i) { return i.id === that.inputPatientId; });
...
targetPatients.forEach(function(i) { i.id = that.outputPatientId; });
...
//        console.log(linkObject);
        ...
        } else
        {
            ...
            that.printSummaryOfLoadedData(inputMap);
            var outputMap = that.processLoadedData(inputMap, oldToNewIdMap, that.outputPatientId);

            saveResultPromise = that.saveAllToServer(outputMap, oldToNewIdMap);
        ...
        {
            nextPullDataResultPromise = that.pullDataFromFhir(nextLink, inputMap, currentRecursion + 1, oldToNewIdMap);
        }
...
//    patientId += new Date().getTime();
    ...
    return HTTP.request(objectRequest).then(function (response) {
        ...
        if (code === 200 || code === 201) {
            ...
            console.log('  FAILED ' + code + ' for ' + resource + ', body: ' + objectDataString);
        }
    }, function(err) {
...
FhirCloner.prototype.saveAllToServer = function(outputMap, oldToNewIdMap)
{
    ...
    var invokeSetOfSaveRequests = function(setOfObjects){
        ...
        var allPromiseArray = setOfObjects.map(function (i) { return that.saveObjectToServer(i.objectType, i.object); } );
