...
module.exports.init = function (app) {
    ...
    proxy.on('http-intercept-request', function (cid, req, res, remoteReq, performRequest) {
        ...
        function interceptCreateOrUpdate() {
            Q.fcall(function () {
                ...
                if (req.user.isPhysician) {
                    ...
                    var accepted = body.acceptedNominations; // list of authors whose nominations we have accepted for this resource
                    var rejected = body.rejectedNominations; // list of authors whose nominations we have rejected for this resource
                    resolveNominationsForAuthors(resourceId, 'accepted', accepted);
                    resolveNominationsForAuthors(resourceId, 'rejected', rejected);
                } else {
        ...
        function interceptDelete() {
            Q.fcall(function () {
                if (req.user.isPhysician) {
                    ...
                    resolveNominations(resourceId, 'cascade-deleted');
                } else {
                    // create a nomination for this request
                    var revInclude;
        ...
        function createNomination(nomination) {
            ...
            app.logger.verbose('nomination-proxy: Making request to nomination service: PUT %s (authorId: %s, resourceId: %s, carePlanId: %s)',
                url, nomination.authorId, nomination.resourceId, nomination.carePlanId);
    ...
    proxy.on('http-intercept-response', function (cid, req, res, remoteRes, remoteResBody, performResponse) {
        ...
        function interceptReadOne() {
            parseBody().then(function () {
                resourceId = split[2];
                //    // we don't know the carePlanId... find it
                //    var attr;
                //    if (resourceType === 'Goal') {
                //        attr = 'goal';
                //    } else if (resourceType === 'ProcedureRequest' || resourceType === 'NutritionOrder') {
                //        attr = 'activityreference';
                //    }
                //    var url = app.config.get('proxy_fhir') + '/CarePlan?_format=json&' + attr + '=' + resourceId;
                //    app.logger.verbose('nomination-proxy: Making request to FHIR server: GET', url);
                //    return url;
                //}).then(HTTP.read).then(function (value) {
                //    var result = JSON.parse(value);
                //    return result.entry[0].resource.id;
                //}, function (err) {
                //    if (err.response.status === 404) {
                //        // this is a 404 error, the resourceId does not exist yet
                //        // perhaps this is a Nomination and not yet a resource on the FHIR server?
                //        var url = app.config.get('nomination_service') + '/resources/' + resourceId + '/care-plan';
                //        app.logger.verbose('nomination-proxy: received 404 error from FHIR server, making request to nomination service: GET', url);
                //        return HTTP.read(url).fail(function (err) {
                //            app.logger.error('nomination-proxy: Error when contacting nomination service:', err);
                //            return Q.reject();
                //        });
                //    }
                //    // this is a different error...
                //    app.logger.error('nomination-proxy: Error when contacting FHIR server:', err);
                //    return Q.reject();
                //}).then(function (result) {
                //    carePlanId = result;
                // now, find any nominations that should be attached to this resource
                return injectNominations();
            }).then(function () {
    ...
    // overload
    function resolveNominations(resourceId, resolution) {
        var url = app.config.get('nomination_service') + '/resource-id/' + resourceId;
        deleteNomination(url, resolution);
    }

    function resolveNominationsForAuthors(resourceId, resolution, authorIds) {
        var prefix = app.config.get('nomination_service') + '/author-id/';
        var suffix = '/resource-id/' + resourceId;
