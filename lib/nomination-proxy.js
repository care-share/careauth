...
module.exports.init = function (app) {
    ...
    proxy.on('http-intercept-request', function (cid, req, res, remoteReq, performRequest) {
        ...
        var interceptCreateOrUpdate = function () {
            Q.fcall(function () {
                ...
                if (req.user.isPhysician) {
                    ...
                    return HTTP.read(url).then(function (value) {
                        ...
                        app.logger.verbose('Making request to nomination service: PUT', url);
                        return {
                            url: url,
                            method: 'PUT',
                ...
                app.logger.error('Failed to intercept PUT request:', err);
            }).fin(function () {
                if (req.user.isPhysician) {
    ...
    function resolveNominations(carePlanId, resourceId, resolution, authorIds) {
        ...
        if (authorIds && authorIds.constructor === Array) {
            for (var i = 0; i < authorIds.length; i++) {
                ...
                app.logger.verbose('Making request to nomination service: DELETE %s (nomination %s)', url, resolution);
                // TODO: implement resolution
                HTTP.request({
                    url: url,
                    method: 'DELETE'
                }).fail(function (err) {
                    app.logger.error('  Failed to make request to nomination service!', err);
                }).done();
            }
        }
    }
